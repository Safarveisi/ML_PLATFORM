...
# Processing raw Iris data (${S3_BUCKET} will be filled via envsubst later - see Taskfile.yml)
df = spark.read.csv(
...
# Write the preprocessed spark df into S3 as a single csv file
df1.write.format("csv").option("header", "true").mode("overwrite").save("s3a://${S3_BUCKET}/ml_platform/etl/preprocessed")
